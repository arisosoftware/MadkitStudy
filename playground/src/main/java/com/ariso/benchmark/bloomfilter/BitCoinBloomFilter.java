 ...
 * </p>
 */
public class BitCoinBloomFilter implements Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = -4652775034144264823L;

	/**
	 * The BLOOM_UPDATE_* constants control when the bloom filter is auto-updated by
	 * the peer using it as a filter, either never, for all outputs or only for P2PK
	 * outputs (default)
	 */
	public enum BloomUpdate {
		...
		UPDATE_ALL, // 1
		/**
		 * Only adds outpoints to the filter if the output is a P2PK/pay-to-multisig
		 * script
		 */
	...
	// A filter of 20,000 items and a false positive rate of 0.1% or one of 10,000
	// items and 0.0001% is just under 36,000 bytes
	private static final long MAX_FILTER_SIZE = 36000;
	// There is little reason to ever have more hash functions than 50 given a limit
	// of 36,000 bytes
	private static final int MAX_HASH_FUNCS = 50;

	/**
	 * Constructs a filter with the given parameters which is updated on P2PK
	 * outputs only.
	 */
	public BitCoinBloomFilter(int elements, double falsePositiveRate, long randomNonce) {
	 ...
	 * false positive rate when the given number of elements have been inserted. If
	 * the filter would otherwise be larger than the maximum allowed size, it will
	 * be automatically downsized to the maximum size.
	 * </p>
	 * 
	 * <p>
	 * To check the theoretical false positive rate of a given filter, use
	 * {@link SimpleBloomFilter#getFalsePositiveRate(int)}.
	 * </p>
	 * 
	 * <p>
	 * The anonymity of which coins are yours to any peer which you send a
	 * BloomFilter to is controlled by the false positive rate. For reference, as of
	 * block 187,000, the total number of addresses used in the chain was roughly
	 * 4.5 million. Thus, if you use a false positive rate of 0.001 (0.1%), there
	 * will be, on average, 4,500 distinct public keys/addresses which will be
	 * thought to be yours by nodes which have your bloom filter, but which are not
	 * actually yours. Keep in mind that a remote node can do a pretty good job
	 * estimating the order of magnitude of the false positive rate of a given
	 * filter you provide it when considering the anonymity of a given filter.
	 * </p>
	 * 
	 * <p>
	 * In order for filtered block download to function efficiently, the number of
	 * matched transactions in any given block should be less than (with some
	 * headroom) the maximum size of the MemoryPool used by the Peer doing the
	 * downloading (default is {@link TxConfidenceTable#MAX_SIZE}). See the comment
	 * in processBlock(FilteredBlock) for more information on this restriction.
	 * </p>
	 * 
	 * <p>
	 * randomNonce is a tweak for the hash function used to prevent some theoretical
	 * DoS attacks. It should be a random value, however secureness of the random
	 * value is of no great consequence.
	 * </p>
	 * 
	 * <p>
	 * updateFlag is used to control filter behaviour on the server (remote node)
	 * side when it encounters a hit. See {@link SimpleBloomFilter.BloomUpdate} for
	 * a brief description of each mode. The purpose of this flag is to reduce
	 * network round-tripping and avoid over-dirtying the filter for the most common
	 * wallet configurations.
	 * </p>
	 */
	public BitCoinBloomFilter(int elements, double falsePositiveRate, long randomNonce, BloomUpdate updateFlag) {
		// The following formulas were stolen from Wikipedia's page on Bloom Filters
		// (with the addition of min(..., MAX_...))
	...
	/**
	 * Returns the theoretical false positive rate of this filter if were to contain
	 ...
	 */
	// protected void bitcoinSerializeToStream(OutputStream stream) throws
	// IOException {
	 ...
	 * <a href=
	 * "https://github.com/aappleby/smhasher/blob/master/src/MurmurHash3.cpp">C++
	 * code for the original.</a>
	 */
	 ...
	 * steady state bandwidth usage when you want full blocks. Instead of receiving
	 * all transaction data twice, you will receive the vast majority of all
	 * transactions just once, at broadcast time. Solved blocks will then be send
	 * just as Merkle trees of tx hashes, meaning a constant 32 bytes of data for
	 * each transaction instead of 100-300 bytes as per usual.
	 */
	...
	/**
	 * Copies filter into this. Filter must have the same size, hash function count
	 * and nTweak or an IllegalArgumentException will be thrown.
	 */
	// public synchronized void merge(BitCoinBloomFilter filter) {
	 ...
	 * {@link SimpleBloomFilter#setMatchAll()} for when this can be a useful thing
	 * to do.
	 */
	...
	/**
	 * The update flag controls how application of the filter to a block modifies
	 * the filter. See the enum javadocs for information on what occurs and when.
	 */
	...
	public synchronized boolean equals(Object o) {
		...
		if (o == null || getClass() != o.getClass())
			return false;
		BitCoinBloomFilter other = (BitCoinBloomFilter) o;
